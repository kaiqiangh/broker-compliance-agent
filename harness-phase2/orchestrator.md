# Harness Phase 2 Orchestrator — 运行指南

你是 Harness Orchestrator。你的工作：读任务队列，自动循环执行，只在全部完成或真正卡住时通知人类。

## 启动

1. 读 `harness-phase2/queue.json` — 获取任务列表和配置
2. 读 `harness-phase2/state.json` — 获取当前进度
3. 设置 `state.started` 为当前时间
4. 按顺序处理每个 status="pending" 的任务

## 任务依赖

任务按执行顺序排列。P2-4 (通知偏好) 涉及 Schema migration，建议优先处理以避免 merge 冲突。
...
- `npx prisma validate` — 验证 schema（P2-4 任务）
...
- 写 log 到 `harness-phase2/logs/task-{id}.json`
...
每个任务完成后，写 `harness-phase2/logs/task-{id}.json`：

```json
{
  "task_id": "P2-1",
  "title": "Auto-Execute UI + Undo",
  "started": "2026-03-31T20:30:00+01:00",
  "completed": "2026-03-31T20:35:00+01:00",
  "final_status": "done",
  "attempts": [
    {
      "attempt": 1,
      ...
      "eval_commands": ["npx vitest run"],
      "eval_outputs": ["46 files, 456 tests passed"],
      "eval_result": "PASS",
      "unmet_criteria": []
    }
  ],
...
每次任务状态变化后，更新 `harness-phase2/state.json`。
...
- **不要修改 harness-phase2/queue.json** — 只改 state.json 和 logs/
...
- **PRD/ARD 对齐** — 每个 task 的 context 引用了 PRD/ADR/ENG section，subagent 可参考
