...
/** Escape HTML entities to prevent XSS in notification emails. */
...
export async function sendDailyDigest(firmId: string): Promise<void> {
  ...
  const [emailsProcessed, pendingActions, confirmed, rejected, modified, actions] =
    await Promise.all([
      prisma.incomingEmail.count({
        where: { firmId, status: 'processed', processedAt: { gte: today } },
      }),
      prisma.agentAction.count({ where: { firmId, status: 'pending' } }),
      prisma.agentAction.count({ where: { firmId, status: 'confirmed', confirmedAt: { gte: today } } }),
      prisma.agentAction.count({ where: { firmId, status: 'rejected', createdAt: { gte: today } } }),
      prisma.agentAction.count({ where: { firmId, status: 'modified', confirmedAt: { gte: today } } }),
  ...
  const users = await prisma.user.findMany({
    where: { firmId, isActive: true, role: { in: ['firm_admin', 'compliance_officer'] } },
  ...
  const firmName = esc(firm?.name ?? 'Your Firm');
  const appUrl = esc(process.env.APP_URL || 'https://app.brokercomply.ie');

  const html = `<!DOCTYPE html><html>
<body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <h2 style="color: #2563eb;">Agent Daily Digest — ${firmName}</h2>
  ...
  ${pendingActions > 0 ? `<p><a href="${appUrl}/agent" style="display: inline-block; padding: 12px 24px; background: #2563eb; color: white; text-decoration: none; border-radius: 6px;">Review ${pendingActions} Pending Action${pendingActions > 1 ? 's' : ''}</a></p>` : '<p>All caught up.</p>'}
</body></html>`;
  ...
  for (const user of users) {
    try {
      await emailService.send({
        to: user.email,
        subject: `Agent Daily Digest: ${pendingActions} pending, ${emailsProcessed} processed`,
        html,
      });
    } catch (err) {
...
export async function sendAutoExecuteNotification(firmId: string, actionId: string): Promise<void> {
  ...
  const firm = await prisma.firm.findUnique({ where: { id: firmId }, select: { name: true } });
  const firmName = esc(firm?.name ?? 'Unknown');
  const emailSubject = esc(action.email?.subject ?? 'N/A');
  const emailFrom = esc(action.email?.fromAddress ?? 'N/A');
  const appUrl = esc(process.env.APP_URL || 'https://app.brokercomply.ie');
...
<body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>Firm</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${firmName}</td></tr>
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>Action Type</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${esc(action.actionType)}</td></tr>
    ...
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>Email Subject</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${emailSubject}</td></tr>
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>From</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${emailFrom}</td></tr>
  </table>
  <p><a href="${appUrl}/agent/actions/${esc(action.id)}" style="display: inline-block; padding: 12px 24px; background: #2563eb; color: white; text-decoration: none; border-radius: 6px;">Review &amp; Undo if Needed</a></p>
</body></html>`;

  for (const user of users) {
    try {
      await emailService.send({ to: user.email, subject: `Auto-executed: ${esc(action.actionType)} — review within 24h`, html });
    } catch (err) {
...
export async function sendUrgentNotification(firmId: string, actionId: string): Promise<void> {
  ...
  const isUrgent = URGENT_ACTION_TYPES.includes(action.actionType) || confidence < 0.5;
  if (!isUrgent) return;
...
<body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>${esc(reason)} — immediate review recommended.</p>
  <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>Firm</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${firmName}</td></tr>
    <tr><td style="padding: 8px; border: 1px solid #e5e7eb;"><strong>Action Type</strong></td><td style="padding: 8px; border: 1px solid #e5e7eb;">${esc(action.actionType)}</td></tr>
  ...
  <p><a href="${appUrl}/agent/actions/${esc(action.id)}" style="display: inline-block; padding: 12px 24px; background: #dc2626; color: white; text-decoration: none; border-radius: 6px;">Review Action Now</a></p>
</body></html>`;

  for (const user of users) {
    try {
      await emailService.send({ to: user.email, subject: `URGENT: ${esc(reason)} — action needs review`, html });
    } catch (err) {
...
/**
 * Check accuracy trend — alert if accuracy declined 3 consecutive days.
 * BUG-07 FIX: comparison operator was inverted (alerted on improvement, not decline).
 */
export async function checkAccuracyTrend(firmId: string): Promise<void> {
  ...
  // BUG-07 FIX: check for DECLINING accuracy (accuracies[i] > accuracies[i+1] means declining)
  ...
  const html = `<!DOCTYPE html><html>
<body style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  <h2 style="color: #dc2626;">⚠️ Agent Accuracy Declining — ${firmName}</h2>
  <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
    ${metrics.map((m) => {
      const decided = m.actionsConfirmed + m.actionsRejected + m.actionsModified;
      const acc = decided > 0 ? Math.round(((m.actionsConfirmed + m.actionsModified) / decided) * 100) : 'N/A';
      return `<tr><td style="padding: 8px; border: 1px solid #e5e7eb;">${esc(m.date.toISOString().split('T')[0])}</td><td style="padding: 8px; border: 1px solid #e5e7eb;">${acc === 'N/A' ? 'N/A' : acc + '%'}</td></tr>`;
    }).join('\n    ')}
  </table>
  ...
  for (const user of users) {
    try {
      await emailService.send({ to: user.email, subject: `⚠️ Agent accuracy declining for 3 days — ${firm?.name ?? 'Your Firm'}`, html });
    } catch (err) {
